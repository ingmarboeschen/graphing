...
#' @export
#' @example
#' x<-sample(2010:2020,10000,T,.2+(1:11)*.5)
#' y<-sample(paste("category",LETTERS[1:10]),10000,T)
#' par(mar=c(4,4,2,5))
...
# prepare color
   if(is.null(col)){
      ...
      col<-col[10:1]
      col<-rep(col,length.out=dim(x)[2])
   }
...
if(sum(mat!=round(mat))>0) axis(2,seq(0,max(ylim),len=5),format(round(seq(0,max(ylim),len=5),4),big.mark=","),cex=cex.axis,las=las)
# draw polygons
