...
# add legend
 if(legend==TRUE){
    ...
    if(max(as.numeric(mat[,3]))==1) text<-paste(1:3,"connect.")
...
#    text(0,120+legend1adjust,"n connections")
  ...
  # output frequency table of detected countries
    return(sort(table(unlist(country))))
}
...
rename.country<-function(x){
  ...
  x<-gsub("United States|United States of America","United States of America",x)
